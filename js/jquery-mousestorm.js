...
(function($) {
	$.fn.mousestorm = function(options) {
		...
		//创建style标签，加入文档
		var positontype = 'absolute'
		if(opts.particle){
			var positontype = 'absolute'
		}
		
		$('head').append($('<style id="mouseStorm">'+this.selector+' img{position:'+positontype+';left:0;top:0; transform:rotate(0deg); z-index: 999;}</style>'));
		...
		function createkeyframes() {
			...
			for(var i = 0; i < opts.maxkey; i++) {
				...
				var imgopts = {
					css: {
						transform: "translateX(0) translateY(0) rotate(" + rotate + "deg) scale(" + scale + ")",
						opacity: opacity,
				...
				//stay 为类名前缀
				//判断方向与值
				if(opts.particle&&opts.particle.y=='top'){
					if(newy<0){newy=-1*newy}
				}else if(opts.particle&&opts.particle.y=='bottom'){
					if(newy>0){newy=-1*newy}
				}
				
				targetPosKeyframe += "@keyframes strom" + i + "{ 0%{transform:" + imgopts.css.transform + "; opacity:" + opacity + ";scale(" + scale + ");} 100%{transform: translateX(" + newx + "px) translateY(" + newy + "px) rotate(" + rnd(0, 1080) + "deg) scale(" + rnd(opts.scale[0], opts.scale[1]) + "); opacity:0;} }" + ".stay" + i + "{transform:" + imgopts.css.transform + "; opacity:" + imgopts.css.opacity + ";animation:" + imgopts.css.animation + ";}";
			}
			//将样式添加进文档
				...
				if(parseInt(rnd(0, (1 / opts.numpresent))) == 1) {
					...
					switch(opts.particle.x){
						...
						case 'center':
							...
							break;
						default:
							x = rnd(0,$(window).width());
							break;
					};
					
					switch(opts.particle.y){
						case 'top':
							y = 0;
							break;
						case 'bottom':
